document.addEventListener('DOMContentLoaded', () => {
  ...
  const camera = new THREE.PerspectiveCamera(12, WIDTH / HEIGHT, 1, 10000);
  ...
  function render() {
    ...
    const fixedVector = (new THREE.Vector3(0, 0, 1)).multiplyScalar(1);
    const movingVector = (new THREE.Vector3(0, 0, 1)).multiplyScalar(1.5);

    const t = +new Date();
    movingVector.applyMatrix4(new THREE.Matrix4().makeRotationY((0.0005*t) % (2*Math.PI)));
